 ...
 * para un despacho específico. Permite seleccionar un despacho y visualiza la ruta,
 * los clientes y la ubicación en vivo del motorista.
 */
...
export default function LiveMapPage() {
  ...
  // Efecto que se suscribe a los cambios de ubicación para un despacho seleccionado.
  useEffect(() => {
    ...
    // Función para obtener los clientes y la ubicación inicial del motorista.
    const fetchDataForDespacho = async () => {
      // 1. Obtener facturas del despacho para saber qué clientes visitar.
      ...
      // @ts-ignore
      facturasDespacho.forEach(factura => {
         // @ts-ignore
      ...
      // 2. Obtener la última ubicación conocida del motorista.
    ...
    // 3. Suscripción a los cambios en tiempo real.
    ...
    // Desuscribirse al cambiar de despacho o al desmontar el componente.
    return () => {
      supabase.removeChannel(channel);
      // Limpiar estados al cambiar de despacho
      setMotoristaLocation(null);
      setCustomerLocations([]);
    };
  }, [selectedDespachoId, allDespachos, toast]);
  
  // Combina todas las ubicaciones a mostrar en el mapa.
  const allPoints = useMemo(() => {
    const points = [];
    if (motoristaLocation) {
      points.push({ ...motoristaLocation, type: 'motorista' });
    }
    points.push(...customerLocations.map(c => ({...c, type: 'customer'})));
    points.push({ location: `POINT(${STARTING_POINT.lon} ${STARTING_POINT.lat})`, name: STARTING_POINT.name, type: 'start' });
    return points;
  }, [motoristaLocation, customerLocations]);
    ...
    <Card className="h-full flex flex-col">
      <CardHeader>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <CardTitle>Mapa de Seguimiento por Despacho</CardTitle>
              <CardDescription>
                Seleccione un despacho para ver la ruta, los clientes y la ubicación del motorista.
              </CardDescription>
            </div>
